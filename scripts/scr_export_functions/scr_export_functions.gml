...
function export_STE() {
	var path = get_save_filename("STE Project|*.ste",global.load + ".ste");
	
	if (path == "" || path == undefined || path == noone) return;
...
function export_FurryNightFunkin(path){
...
function export_GRS2(){
	
	var file_export = get_save_filename("json|*.json","data.json");
	if (file_export == "") return;
	
	var export_data = {};
	export_data.notes = [[]];
	export_data.bpm = global.project.bpm;
	
	for (var i = 0; i < array_length(global.project.notes);i++) {
		
		var aux_count = 0;
		
		for (var k = 0; k < array_length(global.project.notes[i]);k++) {
			if (is_struct(global.project.notes[i][k])) {
	...
	var temp_file_save = file_text_open_write(file_export);
	file_text_write_string(temp_file_save,json_stringify(export_data));
	file_text_close(temp_file_save);
}
